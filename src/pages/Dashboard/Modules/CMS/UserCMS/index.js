...
import { SearchComplaint, getallComplaint, getallcomplaintCategories, getallcomplaintRecordById, getallcomplaintRecordByUserId, getallcomplaintTypes } from '../../../../../api/APIs';
...
function CMSUserDashboard() {
    ...
    const transformDepartmentData = (apiData) => {
        return apiData.map((leave) => ({
            ...
            complaintIssuedDate: moment(leave?.complaintIssuedDate).format("YYYY/MM/DD"),
            complaintStatus: leave?.complaintStatus,
        }));
    };
    const getComplaint = useCallback(async () => {
        try {
            const response = await getallcomplaintRecordByUserId(userData.fkUserId,currentPage, pageSize);
            if (response?.success) {
