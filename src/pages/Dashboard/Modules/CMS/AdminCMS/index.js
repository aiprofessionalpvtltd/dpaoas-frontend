...
function CMSAdminDashboard() {
  ...
  const getComplaint = useCallback(async () => {
    try {
      ...
      if (response?.success) {
        const transformedData = transformComplaintData(
  ...
  const CountInventory = async () => {
    try {
      ...
      if (response?.success) {
        const countPending = response?.data?.inventories.filter(
          (item) => item.status === "in-stock/store"
        ).length;
        const countresolved = response?.data?.inventories.filter(
          (item) => item.status === "repairing"
        ).length;
        const countoutoforder = response?.data?.inventories.filter(
          (item) => item.status === "out of order"
        ).length;
        const issuedCount = response?.data?.inventories.filter(
          (item) => item.status === "issued"
        ).length;
        const disposeOfCount = response?.data?.inventories.filter(
          (item) => item.status === "disposed of"
        ).length;
                  ...
                  <div class="mb-3">
                    ...
                    <Select
                      ...
                      onChange={(selectedOptions) =>
                        formik.setFieldValue("complaineeUser", selectedOptions)
                      }
