...
function CMSInventoryBill() {
  ...
  const getAllInvoiceBillApi = useCallback(async () => {
    try {
      ...
      if (response?.success) {
        const transformedData = transformInventoryBillData(
          response?.data?.inventoryBills
        );
  ...
  const hendleEdit = async (id) => {
    try {
      ...
      if (response?.success) {
        navigate("/cms/admin/inventory/inventory-bill/add", {
          state: response?.data,
        });
      }
    } catch (error) {
  ...
  return (
    <Layout module={true} sidebarItems={CMSsidebarItems} centerlogohide={true}>
      <Header dashboardLink={"/cms/admin/inventory/inventory-bill"} />
      <ToastContainer />
      <div class="row">
        <div class="col-12">
          <CustomTable
