...
import { UpdateNABill } from "../../../../../../api/APIs/Services/LegislationModule.service";
...
function PMBTranslationIntroduceInSenate() {
  ...
  const updateQuestion = async () => {
    ...
    formData.append("billCategory", "Private Member Bill")
    ...
    try {
      const response = await UpdateNABill(fkResolutionID, formData);
      if (response?.success) {
  ...
  const onSubmit = async ({
    ...
    // const userId = userData?.fkUserId;
    const category = "PrivateBill_FromSenate";
    const payload = {
        fkIntroducedInSenateId: fkResolutionID,
    ...
    try {
      ...
      if (response) {
        showSuccessMessage(response?.message);
        setTimeout(() => {
         navigate("/tms/legislation/private-bill-translation/introduce-in-senate")
        }, 1000);
      }
    } catch (error) {
  ...
  const getGovIntroduseSenateBillRemarksByIDs = async () => {
    const category = "PrivateBill_FromSenate";
    try {
      const response = await getGovintroduceInSenateById(
        fkNewResolutionId,
        userId,
        category
      );
...
export default PMBTranslationIntroduceInSenate;
