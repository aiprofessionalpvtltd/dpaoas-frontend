  ...
  const transformFRHistory = (apiData) => {
    return apiData.map((item, index) => ({
      ...
      AssignedDate:
        item?.freshReceipt?.length > 0
          ? moment(
              item?.freshReceipt[item?.freshReceipt?.length - 1]?.createdAt
      ...
      DiaryDate:
        item?.freshReceiptDiaries && item?.freshReceiptDiaries?.diaryDate
          ? moment(item?.freshReceiptDiaries?.diaryDate).format("DD/MM/YYYY")
          : "---",
    }));
  };
