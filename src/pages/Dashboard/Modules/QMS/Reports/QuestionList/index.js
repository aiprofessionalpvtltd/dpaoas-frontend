...
function QMSReportQuestionList() {
  ...
  const formik = useFormik({
    initialValues: {
      ...
      houseLayDate: selectedQuestionList
        ? moment(selectedQuestionList.houseLayDate).toDate()
        : "",
      selectedQuestions: [],
    },
    onSubmit: (values) => {
  ...
  const transformLeavesData = (apiData) => {
    return apiData.map((res, index) => {
      const rowData = {
        ...
        questionListStatus: res.questionListStatus
          ? res?.questionListStatus
          : "active",
  ...
  const getQuestionsListBySessionApi = async (sessionId) => {
    try {
      const response = await getAllQuesListBySession(
        sessionId,
        currentPage,
        pageSize
      );
      if (response?.success) {
        ...
        if (response?.data?.questionList?.length > 0) {
          const StartNo =
            response?.data?.questionList[
              response?.data?.questionList?.length - 1
            ]?.startListNo || 0;
  ...
  const handleSaveList = async () => {
    const questionIds = generatedData?.questions?.map((question) => ({
      id: question.id,
    }));
          ...
          <div class="card-body">
            <div class="container-fluid">
              ...
              <CustomTable
                ...
                showListIcon={true}
                handleList={(item) =>
                  navigate("/qms/reports/question-list/supplementary", {
