...
import { allQuestionSummary, getQuestionGroupDiaries, getUnderProcessQuestions } from "../../../../../../api/APIs/Services/Question.service";

function QuestionPendingUnderProcess() {
  ...
  const formik = useFormik({
    ...
    onSubmit: (values) => {
      getPendingUnderProcessApi(values);
    },
  ...
  //   ) {
  //     getGroupDiariesApi(formik?.values, page);
  //   }
  // };
  ...
  const transformSummaryData = (apiData) => {
    return apiData.map((item, index) => ({
      ...
      currentStatus: item?.questionStatuses?.questionStatus
    }));
  };

  const getPendingUnderProcessApi = async (values) => {
    const Data = {session: values?.sessionNo, questionCategory: values?.category, group: values?.groupNo}
    try {
      const response = await getUnderProcessQuestions(Data)
      if (response?.success) {
  ...
  return (
    <Layout module={true} sidebarItems={QMSSideBarItems} centerlogohide={true}>
      ...
      <Header
        dashboardLink={"/"}
        addLink1={"/qms/reports/question-pending-under-process"}
        title1={"Question Pending Under Process"}
      />
      <div class="container-fluid">
        <div class="card mt-1">
          ...
          >
            <h1>Question Pending Under Process</h1>
          </div>
          <div class="card-body">
            <div class="container-fluid">
              ...
              <div
                ...
                >
                  ...
                  <CustomTable
                    ...
                    hidebtn1={true}
                    data={searchedData}
                    tableTitle="Question Pending Under Process"
...
export default QuestionPendingUnderProcess;
