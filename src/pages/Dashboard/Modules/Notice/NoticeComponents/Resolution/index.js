...
const SentResolutionList = ({ isDashboardData }) => {
  ...
  const getAllResolutionsApi = useCallback(async () => {
    ...
    try {
      if (isDashboardData) {
        ...
        response = await getAllCurrentDateResolution(
          currentPage,
          pageSize,
          data
        );
      } else {
        response = await getAllResolutionsNotice(currentPage, pageSize);
      }
          ...
          <div class="card-body">
            ...
            <div class="" style={{ marginTop: "20px" }}>
              <CustomTable
                ...
                showSent={true}
                hideDeleteIcon={true}
                // handleDelete={(item) => deleteResolutionApi(item.id)}
                handleEdit={(item) => handleEdit(item.id)}
                handleSent={(item) => sendResolution(item?.id)}
                totalCount={count}
              />
            </div>
          </div>
...
export default SentResolutionList;
