...
import {
  createCommittees,
  UpdateCommittees,
} from "../../../../../../../api/APIs/Services/Committees.service";
...
function LGMSAddEditCommittees() {
  ...
  const handleCreateCommittee = async (values) => {
    const data = {
      ...
      //   fromationDate: values?.formationDate,
      fromationDate:
        values?.formationDate &&
        moment(values?.formationDate).format("YYYY-MM-DD"),
    };
    ...
    try {
      ...
      if (response?.success) {
        ...
        formik.resetForm();
        setTimeout(() => {
          navigate("/lgms/dashboard/manage/committees/list");
        }, 3000);
      }
    } catch (error) {
  ...
  const handleEditCommittee = async (values) => {
    const data = {
      committeeName: values.committeeName,
      committeeType: values.committeeType,
      fromationDate:
    ...
    try {
      const response = await UpdateCommittees(
  ...
  useEffect(() => {
    ...
    if (location?.state && location?.state) {
      formik.setValues({
        ...
        committeeType: location?.state?.CommitteeType || "",
        formationDate:
          (location?.state?.formationDate &&
            moment(location?.state?.formationDate, "DD-MM-YYYY").toDate()) ||
          "",
      });
    }
  ...
  return (
    ...
    >
      <Header
        ...
        title1={"Committees List"}
        // addLink2={"/lgms/dashboard/manage/members/addedit"}
        title2={
          location && location?.state ? "Edit Committees" : "Add Committees"
        }
      />
      <ToastContainer />
...
export default LGMSAddEditCommittees;
