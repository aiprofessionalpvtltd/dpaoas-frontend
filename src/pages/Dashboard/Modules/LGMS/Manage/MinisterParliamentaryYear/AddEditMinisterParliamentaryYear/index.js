...
function LGMSAddEditMininsterParliamentaryYearForm() {
  ...
  const formik = useFormik({
    ...
    onSubmit: (values) => {
      ...
      if (location?.state?.id) {
        ...
        handleCreateMininsterParliamentaryYearsAPI(values);
      }
    },
  });
  ...
  const handleCreateMininsterParliamentaryYearsAPI = async (values) => {
    const data = {
    ...
    try {
      const response = await createMinisterParliamentaryYears(data);
      if (response?.success) {
...
export default LGMSAddEditMininsterParliamentaryYearForm;
