...
const TestingEditSenateBills = () => {
  ...
  useEffect(() => {
    if (singleSenateBillData) {
      ...
      formik.setValues({
        ...
        membertenure: singleSenateBillData?.tenures
          ? {
              value: singleSenateBillData?.tenures?.id,
              label: singleSenateBillData?.tenures?.tenureName,
            }
          : "",
        tenuresMinisters: singleSenateBillData?.tenuresMinisters
          ? {
        ...
        fkParliamentaryYearId:
          singleSenateBillData?.fkParliamentaryYearId || null,
        fkMnaParliamentaryYearId:
          singleSenateBillData?.fkMnaParliamentaryYearId || null,
        // fkParliamentaryYearId:
        ...
        senateBillMinistryMovers: singleSenateBillData?.senateBillMinistryMovers
          ? {
              value:
                singleSenateBillData?.senateBillMinistryMovers[0]?.ministries
                  ?.id,
              label:
                singleSenateBillData?.senateBillMinistryMovers[0]?.ministries
                  ?.ministryName,
  ...
  const UpdateNationalAssemblyBill = async (values) => {
    try {
      ...
      if (values?.billStatusDate) {
        const formattedDate = moment(values?.billStatusDate).format(
          "YYYY-MM-DD"
        );
        formData.append("billStatusDate", formattedDate);
      }
      ...
      if (values?.noticeDate) {
        formData.append(
          "noticeDate",
          moment(values?.noticeDate).format("YYYY-MM-DD")
        );
      }
      ...
      if (values?.introducedInHouseDate) {
        formData.append(
          "introducedInHouseDate",
          moment(values?.introducedInHouseDate).format("YYYY-MM-DD")
        );
      }
      ...
      if (values?.referedOnDate) {
        formData.append(
          "referedOnDate",
          moment(values?.referedOnDate).format("YYYY-MM-DD")
        );
      }
      ...
      if (values?.reportPresentationDate) {
        formData.append(
          "reportPresentationDate",
          moment(values?.reportPresentationDate).format("YYYY-MM-DD")
        );
      }
      ...
      if (values?.memeberNoticeDate) {
        formData.append(
          "memeberNoticeDate",
          moment(values?.memeberNoticeDate).format("YYYY-MM-DD")
        );
      }
      ...
      if (values?.dateOfConsiderationBill) {
        formData.append(
          "dateOfConsiderationBill",
          moment(values?.dateOfConsiderationBill).format("YYYY-MM-DD")
        );
      }
      ...
      if (values?.dateOfPublishInGazette) {
        formData.append(
          "dateOfPublishInGazette",
          moment(values?.dateOfPublishInGazette).format("YYYY-MM-DD")
        );
      }
      ...
      if (values?.dateOfAssentByThePresident) {
        formData.append(
          "dateOfAssentByThePresident",
          moment(values?.dateOfAssentByThePresident).format("YYYY-MM-DD")
        );
      }
      ...
      if (values?.dateOfPassageBySenate) {
        formData.append(
          "dateOfPassageBySenate",
          moment(values?.dateOfPassageBySenate).format("YYYY-MM-DD")
        );
      }
      ...
      if (values?.dateOfTransmissionToNA) {
        formData.append(
          "dateOfTransmissionToNA",
          moment(values?.dateOfTransmissionToNA).format("YYYY-MM-DD")
        );
      }
      ...
      if (values?.dateOfReceiptMessageFromNA) {
        formData.append(
          "dateOfReceiptMessageFromNA",
          moment(values?.dateOfReceiptMessageFromNA).format("YYYY-MM-DD")
        );
      }
      ...
      if (values?.dateOfPassageByNA) {
        formData.append(
          "dateOfPassageByNA",
          moment(values?.dateOfPassageByNA).format("YYYY-MM-DD")
        );
      }
      ...
      // API call with try-catch for error handling
      try {
        const response = await UpdateNABill(GovSenateBillID, formData);
        ...
        if (response?.success) {
          ...
          setTimeout(() => {
            navigate(
              BillCategory === "Government Bill" && BillFrom === "From Senate"
                ? "/lgms/dashboard/bills/legislation-bills/government-bills/introduced-in-senate"
                : "/lgms/dashboard/bills/legislation-bills/private-member-bills/introduced-in-senate"
            );
          }, 3000);
      ...
      console.error("Unexpected error in UpdateNationalAssemblyBill:", error);
    }
  };
...
export default TestingEditSenateBills;
